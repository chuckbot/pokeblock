...
<template>
  <div class="flex justify-center mx-auto mb-16">
    <div class="flex flex-col overflow-x-scroll md:overflow-x-auto">
      <table :id="_uniqueId('')"
        class="divide-y divide-[var(--secondary-color)] border-4 border-[var(--secondary-color)] sm:block">
        <thead class="bg-gray-50 block">
          <tr class="bg-[var(--primary-color)]">
            <th class="px-6 py-2 sm:text-lg text-xs text-[var(--secondary-color)]" v-for="head in props.tableHeader"
              :key="_uniqueId('')">
              {{ head }}
            </th>
          </tr>
        ...
        <tbody
          ...
          <tr class="text-center block" v-for="obj in props.data" :key="_uniqueId('')">
            <td :style="`width:${tdWidth}`" class="px-6 py-4 text-sm text-[var(--secondary-color)] inline-block"
              v-for="data in obj" :key="_uniqueId('')">
              {{ data }}
            </td>
          </tr>
        </tbody>
